...
class GridManager
{
	...
	/**
	 * @public {Boolean} Whether we are on firefox.
	 */
	static firefox = GridManager.firefoxCheck ();
	 ...
	 */
	static verticalCardsMobile = 1.3;
	...
	/**
	 * @public {Number} The delay before beginning flipping when a dealing animation doesn't occur.
	 */
	static noDealFlipDelay = 200;
	...
	)
	{
		...
		/* Only deal if not on firefox */
		if ( !GridManager.firefox )
		{
			/* Create a dealer */
		...
		{
			/* Position the cards */
			new CardAnim ( this._cards, layout.cardPositions, null, "ease-in-out", 0 ).animate ();
			...
			if ( this._setupCallback )
				setTimeout ( () => { this._setupCallback ();this._setupCallback = null; }, GridManager.noDealFlipDelay );
	...
	/**
	 * @see {@link https://stackoverflow.com/questions/7000190/detect-all-firefox-versions-in-js}
	 */
	static firefoxCheck ()
	{
